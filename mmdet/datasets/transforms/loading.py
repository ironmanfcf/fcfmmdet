...
class LoadDualAnnotations(MMCV_LoadAnnotations):
    ...
    #     # Normalize depth map to 0-255 (assuming original depth range is 0-65535)
    #     depth_map = (depth_map / 65536.0) 

    #     # Convert to uint8
    #     depth_map = depth_map.astype(np.float16)

    #     results['gt_seg_map'] = depth_map
    ...
    def _load_seg_map(self, results: dict) -> None:
        ...
        # Read the image using OpenCV to handle PNG format
        depth_map = mmcv.imfrombytes(img_bytes, flag='unchanged', backend='cv2').astype(np.float32)
        # print("最大值:", np.max(depth_map))
        # print("最小值", np.min(depth_map)) 

        # Normalize depth map to 0-1 (assuming original depth range is 0-65535)
        depth_map = (depth_map / 65536.0) 
        # print("最大值 (归一化后):", np.max(depth_map))
        # print("最小值 (归一化后):", np.min(depth_map))

        # Convert to uint8
        depth_map = depth_map.astype(np.float32)
        # print("最大值 (float32):", np.max(depth_map))
        # print("最小值 (float32):", np.min(depth_map))
...
@TRANSFORMS.register_module()
class LoadDualADAnnotations(MMCV_LoadAnnotations):
    """Load and process the ``instances`` and ``seg_map`` annotation provided
    by dataset.
    ...
    def __init__(
            ...
            ignore_index: int = 255,
            **kwargs) -> None:
        super(LoadDualADAnnotations, self).__init__(**kwargs)
        self.with_mask = with_mask
        self.poly2mask = poly2mask
        self.box_type = box_type
        self.reduce_zero_label = reduce_zero_label
